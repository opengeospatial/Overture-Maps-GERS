...
to register and maintain a persistent registery of these unique IDs. This includes a change log so that changes to
geographic entiries over time can be captured and persisted. This system aims to simplify data integration and sharing across different 
...
The OGC GERS Standrd schema reference docs will describe key schema concepts for each theme and definitions and examples for each feature type.
...
define how all the enumarations contained in the schema can be extended while maintaining backwards compatibility.
