:CSname: Global Entity Reference ID (GER-ID) Framework
...
This document provides a justification as to why the OGC Technical Committee (TC) should consider the Global Entity Reference ID (GER-ID) model and 
framework as an OGC Community Standard. This justification, along with the submitted candidate Community Standard, will form the basis for 
...
Once approved, the GER-ID Community Standard Work Item defined by this document is valid for a period of six (6) months from the approval date.
...
NOTE:: The proposed GER-ID Community Standard DOES NOT include requirements for data, tools, processes, or licensing as defined in the original Overture Maps implementation of the framework. There are NO dependencies on any existing implementations of the framework. 
...
The Global Entity Reference System (GERS) was originally developed by the https://overturemaps.org/[Overture Maps Foundation]. The goal of the GERS Model 
and framework is to simplify data integration and sharing across different datasets and platforms, fostering collaboration and innovation in the geospatial industry.

NOTE: Overture provides a complete and operational implementation of the GERS Model that includes data schemas, a registry schema, the GERS ID generation, processes, tools, and the physical foundation reference map maintained by the Overture Map Foundation. However, the Community Standard does not specify or require using the OMF implementation or any other organization's implementation of components of the GERS fraemework.

The proposed OGC GERS Community Standard will have a limited focus on documenting the GERS logical model and associated framework as documented in the current Overture data, bridge, change log, reference map, and registry schemas. This focus defines a standard that can be implemented by any organization that will be consistent with the existing Overture Maps Foundation implementation of the GERS framework but "owned" by the implementing organization.
