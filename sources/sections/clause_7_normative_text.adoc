...
The data contained in a `changelog` is made available in some file or database system. partitioned by theme, type, and change_type and sorted geospatially with a unique index on id. The changelog files are available at the following locations:
...
include::../requirements/REC013-changelog-name.adoc[]
...
In a CDB 2.0 datastore, the path could be /global/changelog/2026-01-21.0
