...
The following defines the requirements for expressing geometry in an implementation of the GERS framework including schemas and distributions.
...
|===
|Column	| Data type	| Description
|id	| string	| GERS conformant identifier (UUID)
...
|first_seen	| string	| Release number when entity first appeared in a GERS framework compliant repository
|last_seen	| string	| Most recent release containing entity/feature
|last_changed	| string	| Last release in which the entity/feature was changed, sourced from ChangeLog
...
NOTE: The Registry can be used to verify that an entity exists in Overture, track the history of that entity, and find new entities that have been added.
