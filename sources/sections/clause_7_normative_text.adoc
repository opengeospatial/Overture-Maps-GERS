...
include::../recommendations/REC001-schema-recommended.adoc[]
...
of the GERS framework are compliant with ISO 19107: Spatial Schema and the OGC Simple Features Standard. The enumeration of geometry types currently used
...
include::../requirements/REQ003-gers-geometry.adoc[]

For those not familiar with geoemtry types, the following is provided.

The common primary 0, 1, and 2-dimensional objects used in geopatial applications are: 
...
include::../requirements/REQ004-gers-country-code.adoc[]
...
The following requirement states that only non-projected coordinate reference systems are used in an implementation of the GERS framework. The are several reasons for this requirement, one of which is the possible use of global tiled storage systems for a GERS reference map.
...
==== Every GERS entity has a GERS ID
...
==== Example query of a GERS Compliant Registry (From Overture)
...
include::../requirements/REQ014-gers-changelog-name.adoc[]
