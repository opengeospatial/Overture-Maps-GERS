...
An implementation of the GERS model and framework can change this equation by specifying a mechanism for defining persistent, unique identifiers for any geospatial entity as well as a supporting infrastructure such as a registry of those IDs and related metadata.

=== What is in the OGC GERS Community Standard?

The following components of the Global Entity Reference System model and framework are addressed in the GERS Community Standard. The source of the content for the draft Community Standard is the January 2026 of the Overture GERS release. Please note that depending on an organization's requirements, developers may implement one or more components of the framework.

- BridgeFiles: Bridge files connect the unique GERS IDs to the entity identifiers used in the source data. These files are a key component of GERS and offer two critical capabilities: reverse lookup of source features and the ability to join content from multiple geospatial data repositories.
- ChangeLog: A GERS structured changelog capture changes in the map data from the previous release to the current release. This information can be used to guide decisions about data matching, better understand data stability, and help detect data errors.
- GERS ID: The requirements for generating a unique GERS ID. This ID is a unique identifier for real-world geospatial entities across data releases and maintains consistency when the same entity appears in multiple source datasets. 
- ReferenceMap: A reference map of all geospatial entities that have been assigned a unique GERS ID. The content in the reference map may be defined by a set of JSON schema.
- Registry: A GERS Registry serves as the single source of truth for all geospatial entities that are part of an implementation of a Global Entity Reference System.
...
- Feature theme: A GERS compliant reference map structures geospatial content into themes. The Community Standard provides a default enumeration of themes. The theme enumeration may be used as is, extended or replaced with an enumeration as required by the implementating organization or community. 
- Feature Types: Each theme consists of one or more feature types. The GERS framework provides default type enumerations, such as land cover types. The theme/type enumeration may be used as is, extended or replaced with an enumeration as required by the implementating organization or community. 

Each of the above components are defined using JSON Schema that will be clearly and fully documented in the OGC GERS Community Standard. 

=== What will not be in the Community Standard

No Overture maintained data, registry content, maps, tools, or workflow processes will be part of the OGC Community Standard. There is no depemdency on the Overture Referance Map. However, as the Reference Map is freely and openly available for access and use, organizations may choose to utilize content maintained in the OMF implementation or any other implementation that maintains a reference map.
