...
include::../requirements/path-to-reference-map[]

Where the components of the path are:

|===
|<root>     | Implementation dependent but the root location where a path to a set of files begins. Could be `"/"` for a CDB datastore or `s3://store_name/...` in an GeoParquet S3 implementation or an `https://` landing page.
